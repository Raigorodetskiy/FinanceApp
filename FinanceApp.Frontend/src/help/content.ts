    ...
    sections: [
      {
        ...
        blocks: [
          ...
          { type: 'paragraph', text: 'В режиме сортировки «Рост за период» каталог строится как единый глобальный список: порядок зависит только от выбранного периода и направления сортировки.' },
          { type: 'list', items: [
            'Членство в индексах, биржа и факт нахождения в портфеле не влияют на порядок при периодной сортировке.',
            'Значения с доступной доходностью сортируются первыми, записи без доступного значения всегда уходят в конец.',
            'Поиск по названию индекса остаётся обычным текстовым поиском и не является скрытым индекс-фильтром сортировки.',
          ] },
        ],
      },
      {
        ...
        blocks: [
          ...
          { type: 'list', items: [
            'Каждый листинг (ticker + биржа + provider symbol) отображается отдельно: график Frankfurt не объединяется с NYSE/NASDAQ, даже если компания одна и та же.',
            '«24 ч.» = полная предыдущая торговая сессия + текущая торговая сессия выбранного листинга (в локальном времени биржи с учётом выходных/праздников), а не скользящее «сейчас − 24 часа».',
            '«Сегодня» всегда показывает только текущую торговую сессию выбранного листинга.',
            'В «24 ч.» и «Сегодня» межсессионный разрыв (overnight gap) сохраняется: приложение не соединяет предыдущее закрытие с новым открытием и не дорисовывает несуществующие свечи.',
            'В заголовке графика показывается «Данные на …» — это timestamp последней исторической свечи выбранного диапазона.',
            'Если история устарела или источник временно недоступен, показывается предупреждение; при этом сохранённые свечи не затираются пустым графиком.',
            'После успешного обновления котировки (ручного или фонового) новая точка попадает в сохранённую внутридневную историю и затем сверяется с провайдерными свечами при следующем refresh.',
            'Ручное действие «Перезагрузить историю» заново запрашивает дневные и внутридневные интервалы (включая 5m→10m и 1h), поэтому после него quote-derived точки могут быть заменены точными провайдерными свечами.',
          ] },
        ],
      },
      {
        ...
        blocks: [
          ...
          { type: 'list', items: [
            ...
            'Для режимов «24ч» и «Сегодня» сначала используется внутридневная история, а при разреженных данных — текущий snapshot/изменение к предыдущему закрытию.',
            'На выходных и закрытом рынке данные могут обновляться реже: при отсутствии валидной истории и snapshot строка помечается как недоступная.',
            'Если у constituent недостаточно валидных данных, значение доходности помечается как недоступное и сортируется в конце.',
          ] },
        ],
      },
    ],
