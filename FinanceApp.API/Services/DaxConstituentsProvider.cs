...
/// <summary>
/// Provides DAX constituents from a versioned curated snapshot bundled with the repository.
/// Used because Deutsche Börse / STOXX Ltd. do not provide a free, stable public runtime
/// endpoint for index composition without a commercial data licence.
/// </summary>
public sealed class DaxConstituentsProvider : IDaxIndexConstituentsProvider
{
    public const string CuratedProviderName = "Deutsche Börse / STOXX Ltd. (curated snapshot)";
    private const string SnapshotRelativePath = "Data/index-constituents/dax.curated.snapshot.json";

    private readonly IWebHostEnvironment _environment;
    private readonly ILogger<DaxConstituentsProvider> _logger;
    private readonly string _baseDirectory;

    public DaxConstituentsProvider(
        IWebHostEnvironment environment,
        ILogger<DaxConstituentsProvider> logger,
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (!File.Exists(snapshotPath))
            {
                return IndexConstituentsResult.Failure(
                    ProviderName,
                    "Curated snapshot DAX не найден в приложении.");
            }

            var json = await File.ReadAllTextAsync(snapshotPath, cancellationToken);
            var snapshot = JsonSerializer.Deserialize<DaxCuratedSnapshot>(json);
            if (snapshot is null)
            {
                return IndexConstituentsResult.Failure(
                    ProviderName,
                    "Curated snapshot DAX повреждён или пуст.");
            }
        ...
        {
            _logger.LogError(ex, "Failed to load curated DAX snapshot");
            return IndexConstituentsResult.Failure(
                ProviderName,
                "Не удалось загрузить curated snapshot DAX.");
        }
    }
        ...
        out IReadOnlyList<IndexConstituentEntry> entries)
    {
        ...
        if (string.IsNullOrWhiteSpace(snapshot.SourceUrl))
            return "Curated snapshot DAX не содержит sourceUrl.";

        if (snapshot.AsOfDate == default)
            return "Curated snapshot DAX не содержит корректную asOfDate.";

        if (snapshot.Constituents is null || snapshot.Constituents.Count == 0)
            return "Curated snapshot DAX не содержит constituents.";
        ...
        foreach (var item in snapshot.Constituents)
        {
            ...
            if (string.IsNullOrWhiteSpace(ticker) || string.IsNullOrWhiteSpace(companyName))
                return "Curated snapshot DAX содержит пустой ticker или companyName.";

            if (string.IsNullOrWhiteSpace(providerExchange))
                return $"Curated snapshot DAX содержит пустую биржу для тикера {ticker}.";

            if (!StockExchanges.TryNormalize(providerExchange, out var normalizedExchange))
                return $"Curated snapshot DAX содержит неподдерживаемую биржу '{providerExchange}' для тикера {ticker}.";

            if (isin is not null && !StockIdentifiers.IsValidIsin(isin))
                return $"Curated snapshot DAX содержит некорректный ISIN для тикера {ticker}.";
            ...
            if (providerSymbol.Length == 0)
                return $"Curated snapshot DAX содержит пустой providerSymbol для тикера {ticker}.";

            var uniqueKey = $"{providerSymbol}|{normalizedExchange}";
            if (!uniqueKeys.Add(uniqueKey))
                return $"Curated snapshot DAX содержит дубликат identity: {uniqueKey}.";
    ...
    private sealed class DaxCuratedSnapshot
    {
        ...
        public List<DaxConstituentItem> Constituents { get; init; } = [];
    }

    private sealed class DaxConstituentItem
    {
